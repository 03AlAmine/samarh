...
export class ListServicesComponent implements OnInit {
  ...
  tauxPresenceService = computed(() => {
    ...
    const presents = employes.filter(e => this.presencesAujourdhui().includes(e.matricule)).length;
  ...
  ngOnInit(): void {
    this.employeService.services$
      ...
      .subscribe(s => { this.allServices.set(s); this.loading.set(false); });
  ...
  openDetail(s: Service): void { this.selectedService.set(s); }
  closeDetail(): void { this.selectedService.set(null); }
  ...
  openForm(service?: Service): void {
    ...
    if (service) {
      this.form.patchValue({
        nom: service.nom, matricule: service.matricule || '',
        type_service: service.type_service || 'Permanent',
        description: service.description || '', actif: service.actif !== false,
      });
    } else {
  ...
  closeForm(): void { this.showForm.set(false); this.editingService.set(null); }
  ...
  async deleteService(id: string, event: Event): Promise<void> {
    ...
    const ok = await this.confirm.ask('Supprimer ce service définitivement ? Les employés assignés ne seront pas supprimés.', 'Supprimer', 'danger', 'Supprimer le service');
    if (!ok) return;
    try {
      await this.employeService.deleteService(id);
      this.toast.success('Service supprimé');
    } catch (e: any) { this.toast.error(e.message || 'Erreur.'); }
  }
  ...
  async savePlanningService(planning: Planning[]): Promise<void> {
    ...
    try {
      ...
      this.allServices.update(list =>
        list.map(x => x.id === s.id ? { ...x, planning } : x)
      );
